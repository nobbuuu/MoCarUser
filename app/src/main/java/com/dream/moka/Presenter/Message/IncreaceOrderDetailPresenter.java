 ...
 * @date 2016/8/6.
 */
public class IncreaceOrderDetailPresenter extends RxPvPresenter<OrderAddDetailContract.View> implements OrderAddDetailContract.Presenter<OrderAddDetailContract.View>{

    private ApiService mApiService;

    @Inject
    public IncreaceOrderDetailPresenter(ApiService bookApi) {
        this.mApiService = bookApi;
    }

    @Override
    public void getMyOrderIncrease(String token, String id) {
        addObserver(mApiService.getIncreaseOrderDetail(token,id), new BasePvObserver<OderAddDetailBean>(mView) {
            @Override
